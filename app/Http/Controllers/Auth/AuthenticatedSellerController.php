    ...
    public function store(SellerLoginRequest $request): RedirectResponse
{
    ...
    if (auth()->guard('seller')->attempt($credentials)) {
        ...
        // Check if this is the first login (username is used as password)
        if ($seller->username === $request->input('password')) {
            $request->session()->put('change_password', true); // Mark session for password change
            return redirect()->route('seller.change_password'); // Redirect to ChangePassword
        }
